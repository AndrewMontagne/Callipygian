...
/obj/machinery/power/generator/verb/rotate()
	...
	if ( anchored || usr.stat || usr.restrained() || !usr.canmove )
		return
...
/obj/machinery/power/generator/attackby(obj/item/C, mob/user, params)
	add_fingerprint(user)

	if(istype(C, /obj/item/wrench))
		return wrench_act(user, C)
	else if(istype(C, /obj/item/device/multitool))
		return multitool_act(user, C)
	else if(istype(C, /obj/item/screwdriver))
		return screwdriver_act(user, C)
	else if(istype(C, /obj/item/crowbar))
		return crowbar_act(user, C)

// NOTE: When tgstation/tgstation#35095 is ported, remove attackby and remove proc from the following

/obj/machinery/power/generator/proc/wrench_act(mob/living/user, obj/item/I)
	if(!panel_open)
		return
	anchored = !anchored
	//I.play_tool_sound(src)
	playsound(get_turf(src), I.usesound, 50, 1) //Replace with above when 35095 is ported
	if(!anchored)
		kill_circs()
	connect_to_network()
...
/obj/machinery/power/generator/proc/screwdriver_act(mob/user, obj/item/I)
...
/obj/machinery/power/generator/proc/crowbar_act(mob/user, obj/item/I)
